...
class BBC_Latest(Crawler):
    ...
    def crawl(self, url, articles=set(), videos=set()):
        '''
        ...
        if pagenumber < 42:
            ...
            articles, videos = self.crawl(newlink, articles, videos)
...
class BBC(WaybackCrawler):
    ...
    def get_archive_urls(self):
        return self.get_urls(self.start_url,articles=set(), videos=set())
    def get_urls(url, articles=set(), videos=set()):
        """
        This function takes a URLs and returns lists of URLs
        for containing each article and video on that page.
